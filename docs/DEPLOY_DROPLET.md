...
`TGCLOUD_TOKEN`). Токен вписывается в `.env` (переменная `BOT_TOKEN`).

### 2. Опубликовать Mini App на GitHub Pages

Мини-приложение — статика из `miniapp/`, её не отдаёт сервер. Она публикуется
на **GitHub Pages** отдельным workflow:

1. Включите Pages в репозитории: **Settings → Pages → Source: GitHub Actions**.
2. При пуше изменений в `miniapp/**` workflow
   ([`.github/workflows/pages.yml`](.github/workflows/pages.yml)) сам публикует
   мини-приложение на `https://<username>.github.io/<repo>/`.
3. Убедитесь, что `MINIAPP_URL` в `lib/config.js` указывает на ваш Pages-URL.

### 3. Подключить Mini App к боту

В **@BotFather** → ваш бот → **Menu Button** укажите ссылку на Pages-URL
мини-приложения — у пользователей появится кнопка **«🔍 Найти и следить»**.

#### Выбор городов события в мини-приложении

Чтобы при «Найти и следить» событие предлагало выбрать город, добавьте к
`MINIAPP_URL` в `lib/config.js` параметр `&api=https://ВАШ-ДОМЕН` — публичный
`TICKET_DOMAIN` с droplet, где бот слушает `/api/event-cities`. Без него событие
добавляется без выбора города.
