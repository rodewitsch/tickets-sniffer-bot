...
export async function addWatchItem({ chatId, kind = 'query', source = 'all', query, eventUrl = null, title = null }) {
  ...
  const existing = await db
    ...
    .all();
  const dup = existing.find((i) => i.kind === kind && normText(i.query) === normText(q));
  if (dup) return { row: dup, duplicate: true };

  const [row] = await db
    .insert(watchItems)
    .values({ chatId, kind, source, query: q, eventUrl, title: title || q })
    .returning();
