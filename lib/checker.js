  ...
  for (const src of sources) {
    if (src === 'ticketpro') {
      ...
      for (const ev of found.slice(0, MAX_RESULTS_PER_QUERY)) {
        // Если выбран конкретный город — фильтруем события Ticketpro по городу.
        if (city && city !== 'all') {
          const evCity = normText(ev.city || '');
          const want = normText(cityLabel(city));
          if (!evCity || evCity !== want) continue;
        }
        // Режим «все города» для ключевого слова откладываем (дорого): не фильтруем.
...
async function checkVenueItem(item, budget) {
  ...
  let notified = 0;
  const city = item.city || null;
  const venueUrl = city ? venueUrlForCity(item.eventUrl, city) : item.eventUrl;
