...
async function checkVenueItem(item, budget) {
  ...
  // поэтому афишу берём из schedule API по числовому id площадки (город площадки
  // определяется бэкендом — выбирать город вручную не нужно).
  let notified = 0;
  const { venue, events: found } = await fetchAfishaVenueEvents(url, { query: item.query });
