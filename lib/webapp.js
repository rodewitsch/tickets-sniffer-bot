...
import { fetchAfishaVenue, afishaVenueListingUrl, afishaWorkingEventUrl } from './sources/afisha.js';
...
// Постер с мини-приложения — только абсолютный http(s) URL, иначе null.
...
export async function handleWebAppData(chatId, rawData) {
  ...
  if (payload.a === 'add') {
    ...
    let city = sanitizeCity(payload.c);
    let eventUrl = payload.u || null;
    // Площадка «афиши»: уточняем настоящий город площадки и канонизируем ссылку
    // на рабочую страницу /ru/object/<slug> (старая …/events/<slug> не работает).
    if (kind === 'venue' && source === 'afisha' && eventUrl) {
      try {
        const venue = await fetchAfishaVenue(eventUrl);
        if (venue) {
          city = sanitizeCity(venue.citySlug) || city;
          eventUrl = afishaVenueListingUrl(venue) || eventUrl;
        }
      } catch (e) {
        console.warn('[webapp] afisha venue resolve failed:', e && e.message || e);
      }
    }
    // Событие «афиши»: ссылка на страницу может быть «битой» (SSR 500 на 24afisha.by) —
    // заменяем на рабочую, чтобы кнопка «Открыть» в списке вела куда надо.
    if (kind === 'event' && source === 'afisha' && eventUrl) {
      try {
        eventUrl = await afishaWorkingEventUrl(eventUrl);
      } catch (e) {
        console.warn('[webapp] afisha event url resolve failed:', e && e.message || e);
      }
    }
    try {
      const res = await addWatchItem({
        ...
        image: payload.m ? sanitizeImage(payload.m) : null,
      });
      if (!res) {
