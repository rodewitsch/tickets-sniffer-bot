...
import { fetchAfishaVenue, afishaVenueListingUrl } from './sources/afisha.js';
...
export async function handleWebAppData(chatId, rawData) {
  ...
  if (payload.a === 'add') {
    ...
    // Площадка «афиши»: публичные страницы площадок на 24afisha.by не работают,
    // поэтому уточняем настоящий город площадки и заменяем ссылку на рабочий
    // фильтр афиши ?objectsIds=<id> (по числовому id площадки). Если не вышло —
    // оставляем то, что прислало мини-приложение, и слежение всё равно работает
    // (id площадки уже зашит в URL).
    if (kind === 'venue' && source === 'afisha' && eventUrl) {
      try {
        const venue = await fetchAfishaVenue(eventUrl, { query: payload.q || payload.t });
        if (venue) {
