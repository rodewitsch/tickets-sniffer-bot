...
// Категория события (кино/театр/концерт/…) в выдаче поиска BezKassira не
// выводится, а URL события плоский (без раздела). Точный раздел есть только в
// хлебных крошках страницы события: «Афиша › <Категория> [в городе] › … › событие»,
// где ссылка категории ведёт на /events/<slug>[-<город>]/… Мини-приложение
// просит показывать тип, как у bycard, поэтому живой поиск дополнительно
// обогащает результаты: открывает страницу события и определяет раздел.
// Кэш по URL на несколько часов, чтобы повторные нажатия не дёргали сайт.
let bezkassiraCategory = new Map(); // url -> { label, at }
const BEZKASSIRA_CATEGORY_TTL_MS = 6 * 60 * 60 * 1000;
const BEZKASSIRA_CATEGORY_MAX = 500;
...
// Категория события по его странице (с кэшем по URL).
async function fetchBezkassiraCategory(eventUrl) {
  const url = canonUrl(eventUrl);
  if (!url) return null;
  const hit = bezkassiraCategory.get(url);
  if (hit && Date.now() - hit.at < BEZKASSIRA_CATEGORY_TTL_MS) return hit.label;
  let label = null;
  try {
    const { status, text } = await httpGet(pageUrl(url));
    if (status === 200 && text) label = categoryFromBezkassiraPage(text);
  } catch { /* сеть — оставляем null */ }
  if (bezkassiraCategory.size >= BEZKASSIRA_CATEGORY_MAX) {
    const oldest = bezkassiraCategory.keys().next().value;
    bezkassiraCategory.delete(oldest);
  }
  bezkassiraCategory.set(url, { label, at: Date.now() });
  return label;
}

// Обогащает результаты поиска типом события (кино/театр/концерт/…) — открывает
// страницу каждого события и читает хлебные крошки. Порядок сохраняется.
export async function enrichBezkassiraCategories(cards) {
  ...
  const results = await mapLimitBez(
    ...
    (u) => fetchBezkassiraCategory(u),
  );
  let j = 0;
  return shown.map((e) => {
    const cat = e && e.url ? (results[j] != null ? results[j] : null) : null;
    if (e && e.url) j++;
