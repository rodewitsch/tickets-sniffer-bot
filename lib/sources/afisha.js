...
import { MAX_CITY_PROBES } from '../config.js';
...
export async function searchAfisha(query) {
  ...
  const afRes = (results[0].status === 'fulfilled' ? parseSearchPayload(results[0].value.data) : { events: [], venues: [] });
  const bcRes = (results[1].status === 'fulfilled' ? parseSearchPayload(results[1].value.data) : { events: [], venues: [] });
