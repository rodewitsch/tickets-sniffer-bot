...
// Числовой id площадки из URL. Мини-приложение добавляет его фильтром
// ?objectsIds=<id> (рабочая ссылка на афишу площадки, см. afishaVenueListingUrl).
export function afishaObjectIdFromUrl(url) {
  const m = /[?&]objectsIds=(\d+)/.exec(String(url || ''));
  return m ? m[1] : null;
}

// Числовой id площадки. Берём из URL, иначе ищем площадку по имени и сопоставляем
// slug из URL со slug найденных площадок (для позиций, добавленных до этой правки).
async function resolveAfishaObjectId(venueRef, query) {
  const fromUrl = afishaObjectIdFromUrl(venueRef);
  if (fromUrl) return fromUrl;
  const wantSlug = afishaSlugFromUrl(venueRef);
  const name = String(query || '').trim();
  if (!name && !wantSlug) return null;
  const { venues } = await searchAfisha(name || wantSlug);
  const hit = venues.find((v) => v.uid === wantSlug)
    || venues.find((v) => v.uid && wantSlug && v.uid.endsWith(wantSlug));
  return hit && hit.id ? String(hit.id) : null;
}

// Карточка площадки «афиши» по числовому id:
//   GET /api/v2/schedule/objects/<id> → data[0] = { id, name, slug, cityId, events[] }.
// Публичные страницы площадок (/ru/<city>/events/<slug>) на 24afisha.by отдают
// пустую страницу, поэтому за афишей площадки ходим в schedule API.
// Возвращает объект площадки (с сырыми events) или null.
async function fetchAfishaVenueObject(venueRef, query) {
  const id = await resolveAfishaObjectId(venueRef, query);
  if (!id) return null;
  for (const base of [BYCARD_API, API]) {
    try {
      const r = await httpJson(
...
/**
 * Площадка «афиши»: название, город, категория и сырой список событий.
 * Категория нужна для рабочей ссылки на афишу площадки (afishaVenueListingUrl).
 *
 * @param {string} venueRef URL площадки (может содержать ?objectsIds=<id>)
 * @param {{query?:string}} [opts] имя площадки — для поиска id, если его нет в URL
 * @returns {Promise<{id,name,slug,citySlug,categorySlug,events:object[]}|null>}
 */
export async function fetchAfishaVenue(venueRef, { query } = {}) {
  const obj = await fetchAfishaVenueObject(venueRef, query);
  if (!obj) return null;
  const cityIds = await afishaCityIds();
  const citySlug = Object.keys(cityIds).find((k) => cityIds[k] === Number(obj.cityId))
    || afishaCityFromUrl(venueRef);
  const events = Array.isArray(obj.events) ? obj.events : [];
  const categorySlug = events
    .map((e) => e && Array.isArray(e.types) && e.types[0] && e.types[0].slug)
    .find(Boolean) || null;
  return {
    id: obj.id,
    name: obj.name || null,
    slug: obj.slug || afishaSlugFromUrl(venueRef),
    citySlug,
    categorySlug,
    events,
  };
 ...
 * Из карточки площадки берём события с хотя бы одним сеансом isSaleOpen и собираем
 * мини-карточки для уведомления/кэша.
 *
 * @param {string} venueRef URL площадки (может содержать ?objectsIds=<id>)
 * @param {{query?:string}} [opts] имя площадки — для поиска id, если его нет в URL
 * @returns {Promise<{venue:object|null, events:object[]}>}
 */
export async function fetchAfishaVenueEvents(venueRef, { query } = {}) {
  const venue = await fetchAfishaVenue(venueRef, { query });
  if (!venue) return { venue: null, events: [] };
  const { id, name, slug, citySlug, categorySlug, events: rawEvents } = venue;
  const events = [];
  for (const e of rawEvents) {
    if (!e || !e.slug || !e.name) continue;
    const session = (e.sessions || []).find((s) => s && s.isSaleOpen);
    if (!session) continue;
    events.push({
      source: 'afisha',
      uid: `${e.slug}@${citySlug}`,
      ...
      city: citySlug,
      dateText: session.dateStr ? `${session.dateStr} ${session.timeStr || ''}`.trim() : null,
      startsAt: session.timeSpending || null,
      url: `${SITE}/ru/${citySlug}/event/${e.slug}`,
      image: e.image?.['300x430'] || e.image?.original || null,
      priceFrom: session.minPrice ? String(Number(session.minPrice) / 100).replace('.', ',') : null,
      priceTo: session.maxPrice ? String(Number(session.maxPrice) / 100).replace('.', ',') : null,
      currency: 'BYN',
      onSale: true,
      status: 'onsale',
    });
  }
  return { venue: { id, name, slug, citySlug, categorySlug }, events };
}

// Рабочая ссылка на афишу площадки: категорийная страница города с фильтром
// ?objectsIds=<id>. Публичные «страницы площадок» на 24afisha.by не работают,
// поэтому ведём на отфильтрованную афишу. Если категорию определить не удалось
// (например, у площадки сейчас нет событий) — null.
export function afishaVenueListingUrl(venue) {
  if (!venue || !venue.id || !venue.categorySlug) return null;
  const city = venue.citySlug || 'minsk';
  return `${SITE}/ru/${city}/events/${venue.categorySlug}?objectsIds=${venue.id}`;
}
