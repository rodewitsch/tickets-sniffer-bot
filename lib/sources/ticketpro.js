...
// Категория события (кино/театр/концерт/…) в карточках выдачи поиска не указана
// (event-box__category пуст) — точный тип можно прочитать только из хлебных
// крошек на странице самого события. Мини-приложение просит показывать тип, как
// у bycard, поэтому живой поиск дополнительно обогащает результаты: открывает
// страницу события и берёт последнюю хлебную крошку (напр. фильм «Мятеж» на
// /raznoe/myatezh/ → крошки «Разное › Кино» → категория «Кино»).
// Кэш по URL на несколько часов, чтобы повторные нажатия в поиске не дёргали сайт.
let eventCategory = new Map(); // url -> { label, at }
const EVENT_CATEGORY_TTL_MS = 6 * 60 * 60 * 1000;
const EVENT_CATEGORY_MAX = 500;
// Скольких результатов обогащаем за один поиск и с какой конкурентностью.
const CATEGORY_ENRICH_LIMIT = 10;
const CATEGORY_ENRICH_CONCURRENCY = 3;
...
// Категория из раздела URL (лёгкий fallback, если страницу достать не удалось).
// Раздел идёт первым сегментом пути: /bilety-na-koncert/.., /bilety-v-teatr/..
function categoryFromUrlPath(urlRaw) {
  const p = String(urlRaw || '');
  const rel = p.replace(/^https?:\/\/[^/]+/i, '').slice(1);
...
// Читает категорию события со страницы (последняя хлебная крошка, без корня
// «Ticketpro»). Возвращает строку-тип или null. Кэшируется по URL.
async function fetchTicketproCategory(eventUrl) {
  const url = String(eventUrl || '').replace(/[?#].*$/, '');
  if (!url) return null;
  const hit = eventCategory.get(url);
  if (hit && Date.now() - hit.at < EVENT_CATEGORY_TTL_MS) return hit.label;
  let label = null;
  try {
    const { status, text } = await httpGet(url);
    if (status === 200 && text) {
      const names = [];
      for (const m of text.matchAll(/breadcrumbs__name" itemprop="name">([\s\S]*?)<\/span>/g)) {
        const n = decodeEntities(m[1]);
        if (n) names.push(n);
      }
      // Убираем корень («Ticketpro») и берём последнюю крошку — это и есть тип:
      // «Разное › Кино» → «Кино», «Концерты» → «Концерты».
      const root = normText(names[0] || '');
      for (let i = names.length - 1; i >= 0; i--) {
        const c = names[i].trim();
        if (!c) continue;
        const n = normText(c);
        if (i === 0 && n === root) continue; // только корень — нет категории
        label = c;
        break;
      }
    }
  } catch { /* сеть — оставляем null */ }
  if (eventCategory.size >= EVENT_CATEGORY_MAX) {
    const oldest = eventCategory.keys().next().value;
    eventCategory.delete(oldest);
  }
  eventCategory.set(url, { label: label || categoryFromUrlPath(url), at: Date.now() });
  return eventCategory.get(url).label;
}

// Обогащает результаты поиска категорией (кино/театр/концерт/…) — открывает
// страницу каждого события и читает хлебные крошки. Порядок сохраняется;
// категория кладётся в поле `category` (null, если определить не вышло).
export async function enrichTicketproCategories(cards) {
  const shown = (Array.isArray(cards) ? cards : []).slice(0, CATEGORY_ENRICH_LIMIT);
  const results = await mapLimit(
    shown.map((e) => e && e.url).filter(Boolean),
    CATEGORY_ENRICH_CONCURRENCY,
    (u) => fetchTicketproCategory(u),
  );
  let j = 0;
  return shown.map((e) => {
    const cat = e && e.url
      ? (results[j] != null ? results[j] : categoryFromUrlPath(e.url))
      : null;
    if (e && e.url) j++;
