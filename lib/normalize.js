...
const BEZKASSIRA_ORIGIN = 'https://bezkassira.by';
...
export function normalizeLdEvent(ld, source, pageUrl = null) {
  ...
  if (source === 'afisha') {
    ...
    citySlug = afishaCityFromUrl(url);
  } else {
    city = ld.location?.address?.addressLocality || null;
  }

  // Для afisha uid city-aware: '<slug>@<city>'. Одно событие может находиться
  // в продаже в одном городе и отсутствовать в другом — кэшируем раздельно.
  const uid = source === 'afisha'
    ? `${afishaSlugFromUrl(url)}@${citySlug}`
    : url.replace(/[?#].*$/, '');
...
// Извлекает город-слаг из URL «афиши»:
//   /ru/<city>/event/<slug>          → city
//   /afisha/<city>/<type>/<slug>     → city
// По умолчанию 'minsk' (URL без города/неизвестный формат).
export function afishaCityFromUrl(url) {
  const s = String(url);
  const m = s.match(/\/ru\/([a-z\-]+)\/(?:event|events)\//)
    || s.match(/\/afisha\/([a-z\-]+)\//);
  return (m && m[1]) || 'minsk';
}

// Для одного события (slug) город и в кэше events хранится отдельной записью —
// наличие билетов в другом городе может различаться. Поэтому uid делаем
// city-aware: '<slug>@<city>'. Принимает либо slug, либо полный URL события.
export function afishaUidForCity(slugOrUrl, city) {
  const slug = /^https?:/.test(String(slugOrUrl)) ? afishaSlugFromUrl(slugOrUrl) : String(slugOrUrl);
  const citySlug = (city && city !== 'all') ? city : afishaCityFromUrl(slugOrUrl);
