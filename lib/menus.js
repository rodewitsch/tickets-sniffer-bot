...
export function renderWatchlist(items) {
  ...
  const lines = items.map((i, n) => {
    // Для конкретного события с известной категорией (кино/театр/концерт/…) показываем
    // её вместо обобщённого «событие», чтобы было видно, что это за событие.
    const kindLabel = i.kind === 'event' && i.category ? i.category : (KIND_LABEL[i.kind] || i.kind);
    const src = SOURCE_LABEL[i.source] || i.source;
    const parts = [`${kindLabel}, ${src}`];
