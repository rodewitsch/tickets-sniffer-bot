...
- **Проверка билетов** — сервисом `cron` по случайному расписанию: днём (9:00–21:00) каждые 15–30 мин, ночью — каждые 60–120 мин.
...
Крон-проверка билетов выполняется **внутри docker-сервиса `cron`** по случайному
расписанию (днём 9:00–21:00 каждые 15–30 мин, ночью — 60–120 мин, TZ=Europe/Minsk),
а не через GitHub Actions.
...
Мини-приложение предлагает три режима: «Афиша / ByCard» и «Ticketpro» (живой
поиск событий и площадок) и «Все источники» (отслеживание ключевого слова).
Данные передаются боту через `WebApp.sendData`.
